...
let postMetaCollector = (inArgs) => {
    ...
    return new Promise((resolve, reject) => {
        if (post.published && isUpdatePermission) {
            ...
            // get eyecatch image
            let key = _this.config.popularPosts.eyeCatchImageAttributeName;
            if (key && (key in post)) {
                eyeCatchImage = post[key];
            } else if (post.eyeCatchImage || post.postImage || post.thumbnail || (post.ampSettings && post.ampSettings.titleImage.path) ) {
                eyeCatchImage = post.eyeCatchImage  || post.postImage || (post.thumbnail.indexOf("http") === 0 ? post.thumbnail : path.join(_this.config.root, post.thumbnail)) || ( (post.ampSettings && post.ampSettings.titleImage.path) ? post.ampSettings.titleImage.path : null )
